...
export class MemberModel {
  ...
  async getAuth(member: MemberDto): Promise<MemberDto[] | null> {
    const sqlString: SQLStatement = sql`SELECT * FROM buygo.member WHERE account = ${member.account}`;
  ...
  async getAll(): Promise<MemberDto[]> {
    const sqlString: SQLStatement = sql`SELECT * FROM buygo.member;`;
  ...
  async get(id: number): Promise<MemberDto[]> {
    const sqlString: SQLStatement = sql`SELECT * FROM buygo.member WHERE m_id = ${id}`;
    const data: unknown | unknown[] = await new SQL().query(sqlString);
    if (!Array.isArray(data)) return [];
    return data.map((d: unknown) => new MemberDto(d));
  }
  ...
  async insert(member: MemberDto): Promise<number> {
    const sqlString: SQLStatement = sql`INSERT INTO buygo.member (m_name, phone, account, password, authority) VALUES (${member.name}, ${member.phone}, ${member.account}, ${member.password}, 1);`;
  ...
  async update(member: MemberDto): Promise<boolean> {
    const sqlString: SQLStatement = sql`UPDATE buygo.member SET phone = ${member.phone}, password = ${member.password}, m_name = ${member.name} WHERE account = ${member.account};`;
    const result: unknown | unknown[] = await new SQL().query(sqlString);
